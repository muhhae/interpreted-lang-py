...
class file_button:
    def __init__(self, name, path, depth, index: index_counter):
        ...
        self.button = ctk.CTkButton(
            folder_frame,
            text="  " * depth + "  " + name,
            command=lambda: (self.select(), openFilePath(self.path)),
        )

        self.button.grid(row=index.get(), column=0, padx=2, pady=2, sticky="ew")
...
class folder_button:
    def __init__(self, name, path, depth, index: index_counter):
        ...
        self.button.grid(row=self.index.get(), column=0, padx=2, pady=2, sticky="ew")
        self.button.grid_remove()
        self.depth = depth
        self.listfiles()
        self.button.cget("font").configure(size=14)
...
def save(confirmed=False):
    ...
    if current_file == "" or not os.path.exists(current_file):
        saveFile() if tkmessagebox.askyesno(
            "Save", "Do you want to save your code?"
        ) else None
        return
    if open(current_file, "r").read().strip() == current_doc:
        return
    if not confirmed and not tkmessagebox.askyesno(
        "Save", "Do you want to save your code?"
    ):
        return
...
run_menu = ctk.CTkOptionMenu(
    ...
    values=[
        ...
        "Run interactive in external terminal",
        "Restart Terminal",
    ],
    command=run_on_choice,
    width=100,
)
...
text_box_console.pack(fill=ctk.BOTH, expand=8, padx=5, pady=(5, 0))
...
delay_input = ctk.CTkOptionMenu(
    menu_frame,
    values=["100 ms", "500 ms", "1000 ms", "2000 ms", "3000 ms", "4000 ms", "5000 ms"],
    command=set_refresh_cycle_delay,
    width=100,
)
...
auto_save_delay_input = ctk.CTkOptionMenu(
    menu_frame,
    values=["100 ms", "500 ms", "1000 ms", "2000 ms", "3000 ms", "4000 ms", "5000 ms"],
    command=set_auto_save_delay,
    width=100,
)
...
auto_save_input = ctk.CTkSwitch(menu_frame, text="", command=set_auto_save, width=20)
auto_save_input.grid(row=0, column=5, padx=(10, 0), pady=10, sticky="nsew")
